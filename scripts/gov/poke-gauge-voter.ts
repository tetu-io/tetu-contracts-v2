...
import {TetuVoter__factory} from "../../typechain";
...
async function main() {
  ...
  for (const vote of votes) {
    ...
    console.log(new Date(vote.date * 1000), 'percent: ', (+vote.percent).toFixed(2), 'weight: ', Number(vote.weight).toFixed(), 've: ', veId);
  ...
  console.log('total ve voted', totalWeight.toFixed(), minDate.size);

...
//endregion OpenOcean utils
  ...
  it("test convert and deposit", async () => {
    ...
    const swapTransactionData = await buildSwapTransactionDataForOpenOcean(tokenIn, vaultAsset, amount, helper.address);
    ...
    console.log("convertAndDeposit.1");
    ...
    console.log("convertAndDeposit.2");
  ...
  it("test withdraw and convert", async () => {
    ...
    await TokenUtils.getToken(vaultAsset, signer.address, parseUnits('1', 6))

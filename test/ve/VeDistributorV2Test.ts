...
//     expect(await startNewEpoch(ve, veDist)).eq(true);
//
//     // ------------- test ability to claim tokens on old epochs with maxLock
//
//     await expect(ve.setAlwaysMaxLock(1, true)).revertedWith('CLAIM_REWARDS');
//
//     const newId = Number(await ve.tokenId()) + 1;
//     console.log('newId', newId);
//     await ve.createLock(tetu.address, parseUnits('1'), LOCK_PERIOD);
//     const newBalance = await ve.balanceOfNFT(newId);
//     console.log('newBalance', newBalance);
//
//     await expect(ve.setAlwaysMaxLock(newId, true)).revertedWith('CLAIM_REWARDS');
//
//     // ---------------------------------------------------------------------
//
...
//
//
//   await checkTotalVeSupplyAtTS(ve, 0);
//

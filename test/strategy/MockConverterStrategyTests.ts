...
describe("Mock Converter Strategy tests", function () {
  ...
  before(async function () {
    ...
    const converterRewardTokens: string[] = [];
    const converterRewardAmounts: string[] = [];
    converter = await DeployerUtils.deployContract(signer, 'MockTetuConverter',
      converterRewardTokens, converterRewardAmounts) as MockTetuConverter;

    strategy = MockConverterStrategy__factory.connect(
      (await DeployerUtils.deployProxy(signer, 'MockConverterStrategy')), signer);
    const depositorTokens: string[] = [usdc.address, dai.address];

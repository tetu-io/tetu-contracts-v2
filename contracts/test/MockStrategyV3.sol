...
contract MockStrategyV3 is StrategyBaseV3 {
  ...
  function doHardWork() external override returns (uint earned, uint lost) {
    ...
    if (lastEarned != 0) {
      ...
      if (forwarder != address(0)) {
        ...
        IForwarder(forwarder).registerIncome(tokens, amounts, ISplitter(baseState.splitter).vault(), true);
      }
    }
  ...
  function investedAssets() public view override returns (uint) {
    return IERC20(baseState.asset).balanceOf(address(pool));
  }
  ...
  function _depositToPool(
    ...
    if (_slippage != 0) {
      IERC20(baseState.asset).transfer(controller(), _slippage);
    }
    if (amount - _slippage != 0) {
      IERC20(baseState.asset).transfer(address(pool), amount - _slippage);
    }
  ...
  function _withdrawFromPool(uint amount) internal override returns (
    ...
    pool.withdraw(baseState.asset, amount);
  ...
  function _withdrawAllFromPool() internal override returns (
    ...
    pool.withdraw(baseState.asset, investedAssets());

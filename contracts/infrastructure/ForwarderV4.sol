...
/// @author belbix
contract ForwarderV4 is ReentrancyGuard, ControllableV3, IForwarder {
  ...
  function init(address controller_, address _tetu) external initializer {
    ...
    tetu = _tetu;
    // 1k TETU by default
    tetuThreshold = 1_000 * 1e18;
    // 1%
    toInvestFundRatio = RATIO_DENOMINATOR / 100;
  }
  ...
  function _distribute(address incomeToken) internal {
    ...
    if (tetuBalance != 0) {
      ...
      uint toGauges = tetuBalance - toInvestFund;
